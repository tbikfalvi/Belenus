    ...
    if( boCanBeSaved )
    {
        try
        {
            ...
            if( g_poPrefs->isAutoMailOnPCExpiration() )
            {
                ...
                m_poPatientCard->sendAutoMail( AUTO_MAIL_ON_EXPIRE, AUTO_MAIL_DESTINATION_MAIL, deValidDateTo.addDays( g_poPrefs->getPCExpirationDays()*(-1) ).toString("yyyy-MM-dd"), 0, qslUnitIds.join(",") );
            }
