...
void cDlgPanelUseSelect::refreshTable()
{
    ...
    {
        m_qsQuery = QString("SELECT panelUseId, licenceId, name, useTime, (usePrice/100) as usePrice, archive FROM panelUses WHERE panelIds LIKE '%" + QString::number( m_uiPanelId ) + "%' ");
    }
    else
    {
        m_qsQuery = QString("SELECT panelUseId AS id, name, useTime, (usePrice/100) as usePrice FROM panelUses WHERE panelIds LIKE '%" + QString::number( m_uiPanelId ) + "%' ");
    }
