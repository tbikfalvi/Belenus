...
ALTER TABLE `patientcars` ADD `rfId` varchar(30) NOT NULL AFTER `barcode`;

-- -----------------------------------------------------------------------------------

UPDATE settings SET value=NOW() WHERE identifier="GLOBAL_DATA_UPDATED";
UPDATE settings SET value='1_6_0' WHERE identifier='APPLICATION_VERSION';
