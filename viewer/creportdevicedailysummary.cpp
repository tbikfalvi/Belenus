...
cReportDeviceDailySummary::cReportDeviceDailySummary(QWidget *parent, QString p_qsReportName, bool p_bIsAdmin) : cReport(parent,p_qsReportName,p_bIsAdmin)
{
    m_qsReportName          = tr(" Device daily summary ");
    m_qsReportDescription   = tr( "This report shows daily usage of the selected device on the selected date intervall." );

    _setDataTypeEnabled( true );
    _setDataTypeLabelText( tr("Panel :") );
    ...
    poQueryResult = g_poDB->executeQTQuery( "SELECT panelId, title FROM panels WHERE panelId>0" );

    qslDataTypes << QString( "0|%1" ).arg( tr("<Not selected>") );
    ...
    setFilterDataType( QString( "0|%1" ).arg( tr("<Not selected>") ) );
    setFilterDataTypeList( qslDataTypes.join("#") );
}

void cReportDeviceDailySummary::refreshReport()
{
    ...
    QStringList qslFilterPanel = filterType().split("|");

    QString qsQuery = QString( "SELECT title, SUM(timeReal), SUM(timeLeft), SUM(timeCard), SUM(timeCash), SUM(units), DATE(ledgerTime) FROM panels, ledgerdevice WHERE "
                               "panels.panelId=ledgerdevice.panelId AND "
                               "panels.panelId=%1 AND "
                               "DATE(ledgerTime)>=\"%2\" AND "
                               "DATE(ledgerTime)<=\"%3\" "
                               "GROUP BY DATE(ledgerTime) " ).arg( qslFilterPanel.at(0).toInt() )
    ...
    addSubTitle( qslFilterPanel.at(1) + "<p>" );
